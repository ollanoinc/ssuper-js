...
const ssuperMobileId = createCSSId({
  position: 'fixed',
  bottom: 15,
  right: 20,
  width: 65,
...
const centerClass = createCSSClass({
  position: 'absolute',
  top: '50%',
  left: '50%',
...
function _uninstallSsuper() {
  ...
  if (document.getElementById(ssuperMobileId)) {
    document.getElementById(ssuperMobileId).remove()
  }
}
...
function _installSsuper() {
  ...
  // if mobile
  if (mobile.matches) {
    _installMobileSsuper();
  // otherwise
  } else {
    _installDesktopSsuper();
  }
  // listen for changes
  mobile.addListener(function() {
    if (
      (mobile.matches && document.getElementById(ssuperId)) ||
      (!mobile.matches && document.getElementById(ssuperMobileId))
    ) {
...
function _installMobileSsuper() {
  // ssuper div
  let ssuper = document.createElement('div');
  ssuper.id = ssuperMobileId;
  ssuper.addEventListener('click', function() {
    window.open(
       'https://app.ssuper.co/message/' + (window.ssuper.config.address === undefined ? '' : window.ssuper.config.address),
       '_blank'
    );
  });
...
function _installDesktopSsuper() {
